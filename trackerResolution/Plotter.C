...
void Run(TTree *treeTracks, TFile *output, bool quality, double pLo = 1000, double pHi = 2500) {
  ...
  for(int64_t entry = 0; entry < treeTracks->GetEntries(); entry++) {
    ...
    double y_true = tracks.trueVertexPosY;
    double r_true = sqrt(pow(tracks.trueVertexPosX,2)+pow(tracks.trueVertexPosZ,2)) - R_magic;
    double p_true = tracks.trueVertexMom;
    double theta_y_true = asin(tracks.trueVertexMomY/p_true) * 1e3; 

    double y_reco = tracks.recoVertexPosY;
    double r_reco = sqrt(pow(tracks.recoVertexPosX,2)+pow(tracks.recoVertexPosZ,2)) - R_magic;
    double p_reco = tracks.recoVertexMom;
    double theta_y_reco = asin(-tracks.recoVertexMomY/p_reco) * 1e3; 

    int stn = tracks.station;

    if(tracks.trueVertexTime < g2Period*7) continue;

...
void Run(TTree *tree, TFile *output, bool quality, bool momCuts, bool timeCuts, bool truth) {
  ...
  */
  ...
  InitBranchesTracks2 tracks(tree);
  ...
  for(int64_t entry = 0; entry < tree->GetEntries(); entry++) {
    ...
    double y; double p; double py; double t;

    if(truth) {
      y = tracks.trueVertexPosY;
      p = sqrt(pow(tracks.trueVertexMomX,2) + pow(tracks.trueVertexMomY,2) + pow(tracks.trueVertexMomZ,2));
      py = tracks.trueVertexMomY;
      t = tracks.trueTime;
    } else { 
      y = tracks.recoVertexPosY;
      p = sqrt(pow(tracks.recoVertexMomX,2) + pow(tracks.recoVertexMomY,2) + pow(tracks.recoVertexMomZ,2));
  ...
  for (int i_stn = 0; i_stn < n_stn; i_stn++) {
    ...
    for ( int i_slice(0); i_slice < p_nSlices; i_slice++ ) {
      ...
      thetaY_vs_Y_tracks_fine_p_slices_[i_stn].at(i_slice)->Write();
      thetaY_vs_p_tracks_fine_p_slices_[i_stn].at(i_slice)->Write();
...
int main(int argc, char *argv[]) {
  ...
  // Fill histograms
  Run(tree, fout, quality, momCuts, timeCuts, truth);

...
double ModTime(double time) {
  ...
  double g2fracTime = time / (g2Period*injectionFactor);
  int g2fracTimeInt = int(g2fracTime);
  double g2ModTime = (g2fracTime - g2fracTimeInt) * g2Period*injectionFactor;

  return g2ModTime;

}

/*double InjectionFunc( double *x, double *p )  {
  return p[0] * cos(p[1] *  x[0] + p[2]) + p[3]; 
}*/
...
void Run(TTree *tree, TFile *output, bool quality) {
  ...
  for (int i_stn = 0; i_stn < n_stn; i_stn++) { 
    ...
    wiggle_[i_stn] = new TH1D((stns[i_stn]+"_Wiggle").c_str(), ";Decay time [#mus];Tracks", 2700, 0, 2700*0.148936*injectionFactor);
    ...
    thetaY_vs_time_[i_stn] = new TH2D((stns[i_stn]+"_ThetaY_vs_Time").c_str(), ";Decay time [#mus]; #theta_{y} [mrad] / 149.2 ns ", 2700, 0, 2700*0.148936*injectionFactor, 1000, -TMath::Pi()*boostFactor, TMath::Pi()*boostFactor);
    thetaY_vs_time_mod_[i_stn] = new TH2D((stns[i_stn]+"_ThetaY_vs_Time_Modulo").c_str(), ";t_{g#minus2}^{mod} [#mus]; #theta_{y} [mrad] / 149.2 ns", 41, 0, g2Period*injectionFactor, 1000, -TMath::Pi()*boostFactor, TMath::Pi()*boostFactor);
...
/*  // Injection
  TF1 *injectionFunc = new TF1("injectionFunc",InjectionFunc,7*g2Period*injectionFactor,70*g2Period*injectionFactor,3);
  injectionFunc->SetParNames("A_{i}","#omega_{a}/#sqrt{2}","#phi");//,"offset");
  injectionFunc->SetParameters(10.0,omegaAMagic/injectionFactor,TMath::Pi()/2, );//,xmin);*/
  ...
  for(int64_t entry = 0; entry < nEntries; entry++) {
    ...
    // Injection
    // theta_y = theta_y + injectionFunc->Eval(time);

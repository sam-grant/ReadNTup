...
void Run(TTree *tree, TFile *output, bool quality = true, bool correction = false, string dataset = "Run-1a") {

  // Vertical offset correction verification
  TFile *verticalOffsetFile = TFile::Open(("correctionPlots/verticalOffsetFits_"+dataset+"_125MeV_BQ.root").c_str()); 
  TGraphErrors* verticalOffsetGraph = (TGraphErrors*)verticalOffsetFile->Get("MainPlots/S12S18_ThetaY_vs_Time_Fit");//_"+std::to_string(lo)+"_"+std::to_string(hi)).c_str());
  TF1 *verticalOffsetFunc = (TF1*)verticalOffsetGraph->GetFunction("DoubleExponentialFunc");
  ...
  for (int i_stn = 0; i_stn < n_stn; i_stn++) { 
    ...
    thetaY_vs_time_[i_stn] = new TH2D((stns[i_stn]+"_ThetaY_vs_Time").c_str(), ";Decay time [#mus];#theta_{y} [mrad] / 4.365 #mus", 150, 0, 150*g2Period, 1000, -TMath::Pi()*boostFactor, TMath::Pi()*boostFactor); 
    ...
    for ( int i_slice = 0; i_slice < nSlices; i_slice++ ) { 
      ...
      TH2D *thetaY_vs_time_slice = new TH2D((stns[i_stn]+"_ThetaY_vs_Time_"+std::to_string(lo)+"_"+std::to_string(hi)).c_str(), ";Decay time [#mus];#theta_{y} [mrad] / 4.365 #mus", 150, 0, 150*g2Period, 1000, -TMath::Pi()*boostFactor, TMath::Pi()*boostFactor); 
  ...
  for(int64_t entry = 0; entry < nEntries; entry++) {
    ...
    if(correction) theta_y = theta_y - verticalOffsetFunc->Eval(time);
...
/* string inFileName = "/gm2/data/g2be/Production/Trees/Run1/trackRecoTrees_15921.root";
   ...
   // Fill histograms
   Run(tree, fout, quality, correction, dataset);
